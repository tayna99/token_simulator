...
describe('App AI team operations workspace', () => {
  ...
  it('renders the customer-facing rate card draft in the report panel', async () => {
    ...
    expect(within(rateCard).getByText('draft')).toBeInTheDocument()
    expect(within(rateCard).getByText(/executable: false/i)).toBeInTheDocument()
    expect(within(rateCard).getByText(/connector: connector_not_configured/i)).toBeInTheDocument()
    expect(screen.queryByRole('button', { name: /Execute billing/i })).not.toBeInTheDocument()
    expect(screen.getByRole('button', { name: /Share board-ready PDF/i })).toBeDisabled()
