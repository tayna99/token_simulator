...
describe('runAgentRuntime', () => {
  ...
  it('posts the active stage snapshot to the canonical agentic endpoint', async () => {
    ...
    const result = await runAgentRuntime({
      ...
      ragEvidenceCoverage: {
        officialDocs: { found: true, refs: ['source:google-pricing'] },
      },
      benchmarkEvidenceRefs: ['evidence:artificial-analysis-models'],
      factSources: [],
      operatingAgents: OPERATING_AGENTS.map(agent => ({ ...agent })),
    ...
    expect(JSON.parse(String(requestInit?.body))).toMatchObject({
      ...
      snapshotVersion: 'snapshot:test',
      toolResults: { monthlyAiCogs: 4820 },
      usageLog: [expect.objectContaining({ requestId: 'req-1', modelId: 'gpt-5-mini' })],
      providerModelPriceRefs: [expect.objectContaining({ modelId: 'gpt-5-mini' })],
      costAttribution: expect.objectContaining({ feature: expect.objectContaining({ totalCostUsd: 1.24 }) }),
      marginProfitability: expect.objectContaining({ customers: [expect.objectContaining({ customerId: 'cust-1' })] }),
      optimizationWhatIfSavings: [expect.objectContaining({ id: 'rec-1' })],
      corpusRegistryVersion: 'corpus_registry_v0.1',
      ragEvidenceCoverage: expect.objectContaining({
        officialDocs: expect.objectContaining({ refs: ['source:google-pricing'] }),
      }),
      benchmarkEvidenceRefs: ['evidence:artificial-analysis-models'],
