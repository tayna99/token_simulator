...
export function ReportFirstDiagnosisWorkspace({ workspaceId, productionStatus, fetcher }: Props) {
  ...
  function applySnapshot(
    ...
    setSnapshot(next)
    setSelectedDecisionId(next.decisionCandidates[0]?.id ?? '')
    setPdfArtifact(null)
    setReportError('')
  ...
  async function handleCreateReport() {
    if (!snapshot || !selectedDecisionId || !snapshot.reportGate.canCreateArtifact || !request) return
    setReportError('')
    try {
      const response = await request('/api/reports', {
        ...
        body: JSON.stringify({
          ...
          reportFirst: reportFirstPayloadFromDiagnosis(snapshot, selectedDecisionId),
        }),
      })
  ...
  return (
    <section className="grid gap-6" data-testid="report-first-diagnosis">
      <div className="rounded-wds border border-line-neutral bg-surface-alternative p-5">
        ...
        <h1 className="mt-2 text-3xl font-semibold">AI 비용 리포트 만들기: AI 기능 때문에 손해 보는 고객을 찾으세요</h1>
        <p className="mt-3 max-w-3xl text-sm leading-6 text-label-neutral">
          사용량 CSV/summary 입력으로 OpenAI 비용이 늘었는데 매출은 그대로인 상황을 확인합니다. 어떤 고객과 기능이 마진을 깨는지 찾아드립니다.
        </p>
      ...
      <Surface
        ...
        action={!snapshot ? <Button variant="primary" disabled>PDF 리포트 생성 / PDF 만들기</Button> : undefined}
      >
        ...
        <Surface
          ...
          action={(
            <Button
              ...
              disabled={!snapshot.reportGate.canCreateArtifact || !selectedDecisionId}
            >
              PDF 리포트 생성 / PDF 만들기
            </Button>
          )}
