...
describe('serving economics corpus registry', () => {
  it('registers vLLM benchmark docs as C3 evidence without provider API cost authority', () => {
    ...
    expect(source?.refs).toEqual(['serving:vllm-benchmark-docs'])
    expect(source?.refs).not.toEqual(['evidence:vllm-benchmark-docs'])
  ...
  it('turns serving economics sources into corpus chunks with latency and KV-cache context', () => {
    ...
    const chunk = chunks.find(item => item.id === 'serving:vllm-benchmark-docs')

    expect(chunk).toBeDefined()
    expect(chunk?.corpusId).toBe('serving_economics')
    expect(chunk?.refs).toEqual(['serving:vllm-benchmark-docs'])
    ...
    expect(chunk?.metadata.costAuthority).toBe('self_hosted_serving_economics_only')
    expect(chunk?.metadata.providerApiCostExcluded).toBe(true)
