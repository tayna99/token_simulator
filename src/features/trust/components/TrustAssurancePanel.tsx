...
function buildTrustResultSummary(result: TrustInspectionResult | null | undefined): TrustResultSummary {
  ...
  return {
    ...
    nextAction: isBlocked
      ? '차단된 필드를 제거한 뒤 다시 업로드하세요. 차단된 데이터는 snapshot/report/decision history로 넘어가지 않습니다.'
...
export function TrustAssurancePanel({ result }: Props) {
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        ...
        <span className={`w-fit rounded-wds border px-2 py-1 text-xs font-semibold ${statusTone(summary.status)}`} translate="no">
          {summary.status}
        </span>
      </div>
