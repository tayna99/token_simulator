  ...
  return (
    <section className="bg-white rounded-xl border border-gray-200 p-4 md:p-6">
      <div className="flex items-center gap-2 mb-3">
        <h2 className="text-sm md:text-base font-semibold text-gray-800">{heading}</h2>
        <span className="text-xs text-gray-500" title="Breakdown of costs by token type and impact of optimization options">(?)</span>
      </div>

      <div className="space-y-2 mb-4">
        {rows.map(([label, usd]) => (
          <div key={label} className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 text-sm">
            <span className="w-full sm:w-36 text-gray-600 text-xs sm:text-sm">{label}</span>
            ...
            <span className="w-16 sm:w-20 text-right font-medium text-xs sm:text-sm">
              {fmtCurrency(usd)}
            </span>
            <span className="w-10 sm:w-12 text-right text-xs text-gray-500">
              {percent(usd).toFixed(0)}%
            </span>
          </div>
        ))}
        {br.batchSavingsUsd > 0 && (
          <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 text-sm text-green-700">
            <span className="w-full sm:w-36 text-xs sm:text-sm">{t('costBreakdown.batchSavings')}</span>
