# AI SaaS 비용·마진 Pain Report(문제 리포트)

## 요약

Grok 조사와 Evidence Board(검증 근거 원장) 정리 결과, 이 제품의 핵심 방향은 단순한 LLM(대규모 언어 모델) 비용 계산기가 아니라 **LLM 운영 로그를 고객·기능·모델·플랜·세션 단위 원가로 재분류하고, AI SaaS(AI 기능을 제공하는 구독형 소프트웨어)의 마진과 가격정책 판단으로 연결하는 워크스페이스**다.

결론은 명확하다.

> Group A는 entry point(제품을 처음 쓰게 만드는 진입점), Core Engine(비용 귀속과 의사결정 계산을 맡는 핵심 엔진)은 비용 귀속 레이어, Group B는 paid value(돈을 내고 살 만한 의사결정 가치)다.

개발자 운영 pain(문제)은 빈도가 높다. 하지만 그 자체만으로는 고액 지불 의향이 약할 수 있다. 제품의 핵심은 이 운영 로그를 customer-level cost(고객별 원가), feature cost(기능별 원가), model cost(모델별 원가), plan margin(요금제별 마진), session/agent run cost(세션·에이전트 실행별 원가)로 재분류한 뒤 gross margin(매출총이익률), heavy-user loss(많이 쓰는 고객 때문에 생기는 손실), usage-based pricing(사용량 기준 가격정책), Finance/CEO/Board reporting(재무·CEO·이사회 보고)으로 연결하는 것이다.

## Evidence 상태

현재 공식 원본은 [evidence_board.csv](evidence_board.csv)다.

- 공식 evidence(검증 근거 사례): `GR-001`부터 `GR-038`까지 38개 row-level evidence(행 단위 근거)
...
- budget guardrail(예산 초과를 막는 안전장치)이나 developer diagnostics(개발자용 원인 진단)는 유용하지만, 현재는 research-gated(리서치로 검증된 뒤 재도입할) 기능으로 둔다.

## Core Engine: 비용 Attribution(귀속) 레이어
...
- business denominator(원가를 나눌 비즈니스 기준값)와 판매가를 연결해 원가와 margin(마진)을 계산한다.
- raw cost(순수 API 비용)와 effective cost(재시도·검수·CS 비용까지 포함한 실제 체감 원가)를 분리한다.

제품적 의미:

- 단순히 "비용이 많이 나왔다"가 아니라 "어느 고객/기능/플랜/세션이 마진을 깨는가?"로 질문을 바꾼다.
- Core Engine이 없으면 제품은 개발자 비용 알림 도구에 머문다.
- Core Engine이 있으면 CEO/CFO/PM에게 공유 가능한 unit economics report(단위 경제성 리포트)가 된다.

## Group B: 빈도는 낮지만 WTP가 높은 비즈니스 신호

Group B는 덜 자주 보이지만 WTP(지불 의향)가 큰 신호다. 주로 AI SaaS Founder, CEO, CFO, Finance, PM이 말한다.
...
- CEO/CFO/Board/투자자에게 설명할 AI unit economics(단위 경제성) 지표가 필요하다.

제품적 의미:

- 구매 이유는 "토큰을 아껴준다"보다 "손해 보는 고객과 잘못된 가격정책을 찾는다"에 가깝다.
- MVP(최소 기능 제품)는 cost dashboard(비용 대시보드)보다 margin/pricing decision workspace(마진·가격정책 의사결정 작업공간)로 설명해야 한다.
...
| `pain_heavy_user_loss` | heavy user가 best customer(최고 고객)가 아니라 worst margin customer(마진을 가장 망치는 고객)가 될 수 있다. | heavy-user profitability 분석 |
| `pain_usage_pricing_mismatch` | 비용은 usage 기반인데 가격은 seat/flat이면 손해가 난다. | usage/credit/hybrid pricing simulator(사용량·크레딧·혼합 가격 시뮬레이터) |
...
| P2 | Gateway / Proxy | 장기적으로 routing(요청 경로 선택), guardrail(안전장치), fallback(대체 경로)까지 확장한다. |
...
> 개발자의 LLM 운영 로그를 비즈니스 원가와 가격정책 판단으로 번역하는 AI SaaS unit economics workspace(단위 경제성 워크스페이스)
