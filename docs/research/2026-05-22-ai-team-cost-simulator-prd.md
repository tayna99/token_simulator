...
v0.3은 Wedge A("1인 창업자가 AI 팀을 운영한다")를 **이름**으로 정의했다(AITeamConfiguration, Team Designer). 하지만 그 핵심 메커닉 — **"각 Agent의 입력·출력을 정의하면, 그것으로 토큰과 비용을 계산한다"** — 의 *엔진*은 어느 문서에도 구체화돼 있지 않았다.
...
이 PRD는 그 엔진을 정의한다. 즉 **Agent I/O 명세 → 토큰 추정 → 비용 예측**의 결정론적 모델, 그리고 그것을 감싸는 5화면 MVP.
...
| --- | --- | --- |
| 시점 | **설계 전 (pre-flight)** | 운영 후 (post-hoc) |
...
AI Native Company = AI를 팀원처럼 쓰는 회사. 이런 회사는 **AI 팀의 인건비(=토큰 원가)** 를 관리해야 한다. 이 제품은 그 회사들의 **설계시점 CFO/Ops 레이어**다.
...
`est_tokens`는 **사용자가 토큰 수를 추측하게 하지 않는다**는 원칙(v2.0 §12)을 지키기 위해, Artifact 종류별 **기본 토큰 템플릿 라이브러리**에서 자동 채워진다. 사용자는 "인터뷰 녹취 1건 ≈ 길다/보통/짧다" 수준만 고르거나 그대로 둔다.
...
- **Deterministic(계산)**: 위 산식 전부 — 월 비용, Agent별 비용, 병목 비중, 최적화 후 비용.
