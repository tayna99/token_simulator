# Candidate Collection Log

이 문서는 evidence 후보 수집 명령을 실행했을 때의 관찰 결과를 짧게 남긴다. 후보 전체를 저장하는 문서가 아니라, 어떤 검색이 좋은지 나쁜지 판단하기 위한 로그다.
...
- 후보 5개 확인
- AI IDE/Cursor gross margin 후보가 있었다.
- AI 회사 unit economics/finance 관점 후보가 있었다.
...
- 후보 5개 확인
- business user가 usage-based pricing에서 surprise bill을 피하고 싶다는 후보가 있었다.
...
- 후보 5개 확인
- 관련성이 낮은 issue가 많이 섞였다.
- GitHub는 단순 keyword보다 특정 repo와 query를 좁혀야 한다.
...
- 후보 20개 확인
- AI SaaS pricing, per-analysis cost, gross margin, tenant profitability 후보가 있었다.
- 자동 생성 digest나 무관한 business dashboard issue도 섞였다.
...
- 후보 20개 확인
- Claude Code/Copilot quota issue가 많이 섞였다.
- 사용자별 usage/cost tracking과 finance reconciliation 후보가 있었다.
...
- 후보 20개 확인
- AI SaaS monetization, credit-based billing, per-user LLM cost 후보가 있었다.
...
Historical snapshot before Core Engine attribution expansion:
