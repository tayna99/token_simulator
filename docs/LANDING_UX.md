...
독립 랜딩 구현은 P1에서 보류한다. 별도 랜딩 라우트를 만들지 않는다. 이 문서는 마케팅 랜딩 페이지 구현서가 아니라, 고객이 처음 들어오는 **웹앱 첫 화면**의 온보딩/설명 섹션 참고 문서다.
...
| SparkClaw 역할 | 제출용 샘플 데이터와 데모 시나리오 이름 |
...
첫 화면은 설명형 랜딩이 아니라 곧바로 작업으로 들어가는 dashboard entry다.

| 영역 | 목적 | UI |
| --- | --- | --- |
| Hero | 무엇을 해주는 제품인지 5초 안에 전달 | “내 AI 팀 비용/마진을 5분 안에 보기” |
| CTA | 시작 경로 선택 | 샘플 실행, usage export 업로드, 기존 workspace 열기 |
| Trust note | 원본 prompt/API key/PII를 저장하지 않는다는 신뢰 표시 | Trust Intake badge |
| Workspace preview | 결과가 어디에 쌓이는지 예고 | monthly review history, decision ledger, report export, alert settings |

## 5. CTA 규칙

| CTA | 동작 |
| --- | --- |
| 샘플 실행 | 1인 창업자 샘플 데이터로 Work Ledger, Cost/Margin, Bottleneck, Decision Ledger를 채운다 |
| usage export 업로드 | Trust Intake를 거쳐 실제 usage CSV/adapter export를 normalized usage table로 보낸다 |
